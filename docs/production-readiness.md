...
| Multi-factor authentication | Users can enroll an authenticator or verified email, complete password/OAuth sign-in, use recovery codes, and safely disable or change their factor. | RFC TOTP vectors, handler lifecycle/security tests, isolated PostgreSQL migration and concurrent-consumption tests, and native HTTP enrollment/API recovery checks pass. Real authenticator-app entry, email delivery, desktop/mobile browser interactions, and deployed replica acceptance remain open. |
...
- [ ] Enroll and sign in with both MFA methods using a real authenticator app and
  mailbox. Test resend, recovery codes, an email outage, and factor changes in a
  browser. Upgrade every replica before users enroll MFA.
