...
function uploader(encrypted) {
  ...
  const form={dataset:{maxFiles:'2',maxFileMib:'1',clientEncryption:String(encrypted)},querySelector:s=>fields[s]||null,querySelectorAll:()=>[single,multiple],addEventListener(){}};
  // Without Web Crypto, account uploads stay blocked, but file selection still works.
  vm.runInNewContext(source,{document:{querySelector:()=>form,createElement:element}});
  return {input,selected,multiple,button,zone,access,explanation,list};
}
...
function retryHarness(failure) {
  ...
  const form={dataset:{directUpload:'true',clientEncryption:'true',maxFiles:'3',maxFileMib:'1'},elements:{share_mode:access},querySelector:s=>fields[s]||null,querySelectorAll:()=>[],addEventListener(_name,fn){submit=fn;}};
  ...
  return {submit:()=>submit({preventDefault(){}}),retry:()=>retry.events.click(),input,button,completed,pass,access,key,puts,completions,
