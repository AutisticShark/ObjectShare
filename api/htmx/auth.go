...
type adminUserRow struct {
	ID, Email, DisplayName, Role, CreatedAt, LastLogin string
	...
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if currentIdentity(request) == nil {
			if request.Method == http.MethodGet {
				handler.redirect(writer, request, "/login?next="+url.QueryEscape(request.URL.RequestURI()))
			} else {
...
func (handler *Handler) LoginPage(writer http.ResponseWriter, request *http.Request) {
	...
	handler.render(writer, "login.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: handler.config.Auth.SignupEnabled, Next: safeNext(request.URL.Query().Get("next"))})
}
...
func (handler *Handler) Login(writer http.ResponseWriter, request *http.Request) {
	...
	next := safeNext(request.FormValue("next"))
	...
	if locked {
		...
		handler.render(writer, "login.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: handler.config.Auth.SignupEnabled, Error: "Too many login attempts. Try again in 15 minutes.", Email: request.FormValue("email"), Next: next})
		return
	}
	if user == nil {
		csrf := handler.preAuthCSRF(writer, request)
		if csrf != "" {
			handler.render(writer, "login.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: handler.config.Auth.SignupEnabled, Error: "Email or password is incorrect.", Email: request.FormValue("email"), Next: next})
		}
		return
	}
	if err := handler.startJWT(writer, request, user, true); err != nil {
	...
	handler.redirect(writer, request, next)
}
...
func (handler *Handler) SignupPage(writer http.ResponseWriter, request *http.Request) {
	...
	handler.render(writer, "signup.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: true})
}
...
func (handler *Handler) Signup(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		csrf := handler.preAuthCSRF(writer, request)
		if csrf != "" {
			handler.render(writer, "signup.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: true, Error: err.Error(), Email: request.FormValue("email"), DisplayName: request.FormValue("display_name")})
		}
		return
	}
	hash, err := appauth.HashPassword(password)
	...
	if err := handler.users.CreateUser(request.Context(), user); err != nil {
		if errors.Is(err, db.ErrConflict) {
			csrf := handler.preAuthCSRF(writer, request)
			if csrf != "" {
				handler.render(writer, "signup.html", authPageData{Version: config.GetVersion(), CSRF: csrf, SignupEnabled: true, Error: "That email address is already registered.", Email: email, DisplayName: displayName})
			}
			return
		}
...
func (handler *Handler) renderAccount(writer http.ResponseWriter, request *http.Request, identity *identity, formError, message string) {
	...
	handler.render(writer, "account.html", accountPageData{Version: config.GetVersion(), CSRF: identity.Claims.CSRF, User: identity.User, Files: rows, Error: formError, Message: message})
}
...
func (handler *Handler) UpdateOwnPassword(writer http.ResponseWriter, request *http.Request) {
	...
	if !appauth.VerifyPassword(request.FormValue("current_password"), identity.User.PasswordHash) {
...
func (handler *Handler) AdminUsers(writer http.ResponseWriter, request *http.Request) {
	identity := currentIdentity(request)
	users, err := handler.users.ListUsers(request.Context())
	if err != nil {
	...
	for _, user := range users {
		...
		rows = append(rows, adminUserRow{ID: user.ID, Email: user.Email, DisplayName: user.DisplayName, Role: user.Role,
			Active: user.Active, CreatedAt: user.CreatedAt.UTC().Format("2006-01-02"), LastLogin: lastLogin, IsCurrent: user.ID == identity.User.ID})
	}
	handler.render(writer, "admin_users.html", adminPageData{Version: config.GetVersion(), CSRF: identity.Claims.CSRF, User: identity.User, Users: rows,
		Message: adminMessage(request.URL.Query().Get("message"))})
}
...
func (handler *Handler) AdminCreateUser(writer http.ResponseWriter, request *http.Request) {
	...
	err = handler.users.CreateUser(request.Context(), &db.User{ID: uuid.NewString(), Email: email, DisplayName: displayName, PasswordHash: hash, Role: role, Active: true, TokenVersion: 1})
	if errors.Is(err, db.ErrConflict) {
...
func (handler *Handler) renderAdminError(writer http.ResponseWriter, request *http.Request, identity *identity, message string) {
	...
	rows := make([]adminUserRow, 0, len(users))
	for _, user := range users {
		rows = append(rows, adminUserRow{ID: user.ID, Email: user.Email, DisplayName: user.DisplayName, Role: user.Role, Active: user.Active, CreatedAt: user.CreatedAt.UTC().Format("2006-01-02"), LastLogin: "—", IsCurrent: user.ID == identity.User.ID})
	}
	handler.render(writer, "admin_users.html", adminPageData{Version: config.GetVersion(), CSRF: identity.Claims.CSRF, User: identity.User, Users: rows, Error: message})
}
...
func accountMessage(value string) string {
	return map[string]string{"welcome": "Welcome to ObjectShare.", "profile": "Profile updated.", "password": "Password changed and all earlier JWTs were invalidated."}[value]
}

func adminMessage(value string) string {
	return map[string]string{"setup": "Initial administrator created.", "created": "User created.", "updated": "Access updated; that user's earlier JWTs were invalidated.", "password": "Password reset; that user's earlier JWTs were invalidated.", "deleted": "User deleted."}[value]
}
