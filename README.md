...
Guest uploads are enabled by default. A guest receives a random per-file owner token in an HTTP-only cookie, allowing that browser to rename or delete the file without creating an account. Disable **Allow guest uploads** in the configuration dashboard and save to require login for new uploads; existing unlisted download links and guest owner tokens continue to work.
